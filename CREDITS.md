...
## Imported — animation sources (retargeting in progress)
| Asset | Source | License | Notes |
|---|---|---|---|
| Universal Animation Library[Standard] (`UAL1_Standard.glb`, `UAL1_Standard_RM.glb`, Unity FBX variants) | [Quaternius Universal Animation Library](https://quaternius.itch.io/universal-animation-library) | CC0 | Pack committed at `Universal Animation Library[Standard]/` in the project root. Combat clips not yet retargeted onto `mouse.glb` — see `assets/characters/mouse/QUATERNIUS_RETARGET_SETUP.md` (Godot BoneMap/SkeletonProfileHumanoid). CC0 doesn't legally require attribution, but this project credits sources regardless per standing convention — update this row with the specific clips used once retargeting is done. |
